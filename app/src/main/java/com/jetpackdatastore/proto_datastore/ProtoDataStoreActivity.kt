...
/**
 * Activity represents proto datastore
 */
class ProtoDataStoreActivity : AppCompatActivity() {

    private lateinit var mBinding: ActivityDataStoreBinding

    // We can use dependency injection to inject the ProtoDataStoreManager,
    // but we will keep it simple.
    lateinit var mProtoDataStoreManager : ProtoDataStoreManager
    ...
    private fun initViews() {
        // Initialise ProtoDataStoreManager
        mProtoDataStoreManager = ProtoDataStoreManager(this)
        // Retrieve the saved language from data store
        lifecycleScope.launch {
            val userPreferredLanguage = mProtoDataStoreManager.appLanguage.first()
    ...
    private fun setListeners() {
        mBinding.rgLanguage.setOnCheckedChangeListener { radioGroup, checkedId ->
            ...
            lifecycleScope.launch {
                mProtoDataStoreManager.saveAppLanguage(checkedRadioButton.text.toString())
            }
        }
    }
}
